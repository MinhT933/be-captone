"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PackageCategoriesService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const base_service_1 = require("../base/base.service");
const package_categories_entity_1 = require("./entities/package-categories.entity");
let PackageCategoriesService = class PackageCategoriesService extends base_service_1.BaseService {
    constructor(packgeCategoriesRepository) {
        super(packgeCategoriesRepository);
        this.packgeCategoriesRepository = packgeCategoriesRepository;
    }
    async getAllPackageCategories() {
        return await this.packgeCategoriesRepository.find();
    }
    async createPackageCategories(name, image) {
        const imageRes = await this.uploadImageToFirebase(image);
        return await this.save({
            name: name,
            image: imageRes,
        });
    }
    async getCategoryHasPackageActive() {
        const categoryList = await this.packgeCategoriesRepository
            .createQueryBuilder('package_categories')
            .leftJoinAndSelect('package_categories.packages', 'packages')
            .where('packages.status = :status', { status: 'active' })
            .getMany();
        if (!categoryList || categoryList.length == 0) {
            throw new common_1.HttpException('No package item found', common_1.HttpStatus.NOT_FOUND);
        }
        return categoryList;
    }
    async updatePackageCategory(id, name, image) {
        const category = await this.packgeCategoriesRepository.findOne({
            where: { id: id },
        });
        const imageRes = await this.uploadImageToFirebase(image);
        if (!category) {
            throw new common_1.HttpException(`Category id ${id} not found`, common_1.HttpStatus.NOT_FOUND);
        }
        await this.packgeCategoriesRepository.update({ id: id }, { name: name, image: imageRes });
        return 'Package Category updated';
    }
    async deletePackageCategory(id) {
        try {
            const findCate = await this.packgeCategoriesRepository.findOne({
                where: { id: id },
            });
            if (!findCate || findCate == null) {
                return 'Package category not found!';
            }
            else {
                await this.deleteById(id);
                return 'Delete susccess';
            }
        }
        catch (error) {
            throw new common_1.HttpException('Cannot delete (Get Foreign Key)', common_1.HttpStatus.NOT_FOUND);
        }
    }
};
PackageCategoriesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(package_categories_entity_1.PackageCategoryEntity)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], PackageCategoriesService);
exports.PackageCategoriesService = PackageCategoriesService;
//# sourceMappingURL=package-categories.service.js.map